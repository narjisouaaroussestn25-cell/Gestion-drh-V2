...
st.markdown("""
    ...
    hr {border: 1px solid #FFB347; margin: 0;}
    </style>
...
def get_historique_regroupe():
    ...
    strftime('%d/%m/%Y %H:%M', (SELECT MAX(d2.date_demande) FROM details_demande dt2 JOIN demandes d2 ON dt2.id_demande = d2.id_demande WHERE dt2.id_prestation = p.id_prestation AND dt2.type_mouvement = 'Entrée')) AS 'Date Entrée',
    ...
    strftime('%d/%m/%Y %H:%M', (SELECT MAX(d2.date_demande) FROM details_demande dt2 JOIN demandes d2 ON dt2.id_demande = d2.id_demande WHERE dt2.id_prestation = p.id_prestation AND dt2.type_mouvement = 'Sortie')) AS 'Date Sortie',
...
def get_stock_detaille():
    conn = get_conn()
    query = """SELECT p.designation AS 'Désignation', strftime('%d/%m/%Y %H:%M', (SELECT MAX(d2.date_demande) FROM details_demande dt2 JOIN demandes d2 ON dt2.id_demande = d2.id_demande WHERE dt2.id_prestation = p.id_prestation AND dt2.type_mouvement = 'Entrée')) AS 'Entrée Date',
    ...
    strftime('%d/%m/%Y %H:%M', (SELECT MAX(d2.date_demande) FROM details_demande dt2 JOIN demandes d2 ON dt2.id_demande = d2.id_demande WHERE dt2.id_prestation = p.id_prestation AND dt2.type_mouvement = 'Sortie')) AS 'Sortie Date',
...
def get_mouvements_by_article(id_prestation):
    conn = get_conn()
    query = """SELECT dt.type_mouvement AS 'Type', strftime('%d/%m/%Y %H:%M', d.date_demande) AS 'Date', dt.quantite AS 'Qté', dt.observation AS 'Obs'
    FROM details_demande dt JOIN demandes d ON dt.id_demande = d.id_demande WHERE dt.id_prestation = ? ORDER BY d.date_demande DESC"""
...
def ajouter_prestation(nouvelle_designation):
    ...
    try: 
        cursor.execute("INSERT INTO prestations (designation, categorie) VALUES (?, ?)", (nouvelle_designation, 'Divers'))
        conn.commit()
...
def supprimer_prestation(id_prestation):
    conn = get_conn(); cursor = conn.cursor()
    try: 
        cursor.execute("DELETE FROM details_demande WHERE id_prestation = ?", (id_prestation,))
        cursor.execute("DELETE FROM prestations WHERE id_prestation = ?", (id_prestation,))
        conn.commit()
        return True
    except Exception as e: 
        conn.rollback()
...
def enregistrer_mouvement(id_prestation, type_mv, quantite, observation, datetime_mv):
    conn = get_conn(); cursor = conn.cursor()
    try: 
        cursor.execute("INSERT INTO demandes (date_demande) VALUES (?)", (datetime_mv,))
        id_demande = cursor.lastrowid
        cursor.execute("INSERT INTO details_demande (id_demande, id_prestation, type_mouvement, quantite, observation) VALUES (?, ?, ?, ?, ?)", (id_demande, id_prestation, type_mv, quantite, observation))
        conn.commit()
        return True
    except Exception as e: 
        conn.rollback()
    ...
    if st.session_state.get('confirm_reset', False):
        ...
        if st.checkbox("Je confirme la suppression totale"):
            if st.button("✅ Oui, Supprimer tout"):
                conn = get_conn(); cursor = conn.cursor()
                try: 
                    cursor.execute("DELETE FROM details_demande")
                    cursor.execute("DELETE FROM demandes")
                    cursor.execute("DELETE FROM prestations")
                    conn.commit()
                    st.success("✅ Base réinitialisée! Tout est vide.")
